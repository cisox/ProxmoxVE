...
cd /var/lib/calibre/
$STD curl -fsSL 'https://download.calibre-ebook.com/linux-installer.py' -o linux-installer.py
$STD python3 linux-installer.py
rm -rf linux-installer.py
mkdir -p /var/lib/calibre/library
...
UMask=0002
Type=simple
ExecStart=/usr/bin/calibre-server --port=8080 --enable-local-write /var/lib/calibre/library
TimeoutStopSec=20
KillMode=process
Restart=on-failure
[Install]
